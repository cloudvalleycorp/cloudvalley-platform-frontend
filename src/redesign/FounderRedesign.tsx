import { DemoReportActivity } from "./DemoReportActivity";
import { reportActivityKey } from "./reportActivity";
import { DeleteDemoRecord } from "./DeleteDemoRecord";
import { removeFile } from "./localDocuments";
...
function Workspace() {
  ...
  const [metricView, setMetricView] = useState(() => { try { const saved = localStorage.getItem("rd-metric-view"); return saved === "table" || saved === "grid" ? saved : "cards"; } catch { return "cards"; } });
  const chooseMetricView = (value: string) => { setMetricView(value); try { localStorage.setItem("rd-metric-view", value); } catch { /* Session preference remains available. */ } };
  ...
  const deleteRecord = async (item: RecordItem) => {
    if (!records.some(record => record.id === item.id)) return;
    const next = reconcileTaskEvidence(records.filter(record => record.id !== item.id));
    localStorage.setItem(storageKey, JSON.stringify(next));
    setRecords(next); setSelected(null); setAssistantContext(null); setPage(1);
    if (draft?.editingId === item.id) setDraft(null);
    try {
      localStorage.removeItem(`cloudvalley-redesign-report-${item.id}`);
      localStorage.removeItem(reportActivityKey(item.id));
      if (draft?.editingId === item.id) localStorage.removeItem(`${storageKey}-draft`);
      if (item.area === "sources") await removeFile(item.id);
      setNotice(`${item.name}: eliminado de la demo.`);
    } catch { setNotice(`${item.name}: eliminado. No se pudo limpiar su archivo o borrador local.`); }
  ...
  return <DemoAssistantContextProvider value={askAssistant}><div className="rd-root">
    ...
    <div className="rd-main">
      ...
      <main id="redesign-content" className="rd-content">
        ...
        {area === "overview" ? <>
          ...
          {area === "metrics" && <div className="rd-view-switch" role="group" aria-label="Vista de métricas"><button aria-pressed={metricView === "table"} onClick={() => chooseMetricView("table")}>Tabla</button><button aria-pressed={metricView === "grid"} onClick={() => chooseMetricView("grid")}>Grilla</button><button aria-pressed={metricView === "cards"} onClick={() => chooseMetricView("cards")}>Cards con gráficos</button></div>}
    ...
    <Dialog open={!!selected} onOpenChange={open => !open && setSelected(null)}><DialogContent className={`rd-root rd-dialog rd-detail ${selected?.area === "reports" ? "rd-report-dialog" : ""}`}>{selected && <><div className="rd-detail-icon"><FileText size={24} /></div><DialogTitle>{selected.name}</DialogTitle><DialogDescription>{selected.category} · Datos de demostración</DialogDescription><Badge>{selected.area === "documents" ? documentAccessLabel(selected, records) : selected.status}</Badge><div className="rd-detail-value">{selected.area === "metrics" ? metricDisplay(selected, records) : selected.value}</div><p className="rd-detail-description">{selected.detail}</p><RecordFields item={selected} /><Button variant="ghost" onClick={() => askAssistant({ area: selected.area, record: selected })}><Sparkles size={16} />Consultar sobre este registro</Button><Button disabled={selected.area === "connections" && !self.owner} variant="outline" onClick={() => { setEditing(selected); setWizard(selected.area); setSelected(null); }}>Editar configuración</Button>{(selected.area === "sources" || selected.area === "reports") && <DeleteDemoRecord key={selected.id} item={selected} records={records} onDelete={deleteRecord} />}<div className="rd-detail-security"><LockKeyhole size={16} />Los cambios y accesos se simulan localmente.</div>{selected.area === "reports" && <DemoReportActivity key={selected.id} item={selected} records={records} onAsk={() => askAssistant({ area: "reports", record: selected })} />}{selected.area === "reports" ? <DemoReportEditor onAsk={current => askAssistant({ area: "reports", record: current })} key={`${selected.id}:${assistantRevision}`} item={selected} records={records} onSave={update} onOpenMetric={setSelected} /> : selected.area === "roadmap" && (selected.fields?.requires_doc || selected.fields?.requires_report) ? <TaskEvidence task={selected} records={records} open={setSelected} navigate={next => { setSelected(null); go(next); }} /> : selected.area === "roadmap" ? <Button onClick={() => update({ ...selected, status: selected.status === "Completado" ? "Pendiente" : "Completado" })}><Check size={16} />{selected.status === "Completado" ? "Reabrir tarea" : "Marcar como completada"}</Button> : selected.area === "metrics" ? <MetricValueEntry records={records} onAsk={current => askAssistant({ area: "metrics", record: selected, ...current })} key={selected.id} item={selected} onSave={update} onOpenSource={id => setSelected(records.find(r => r.area === "sources" && r.id === id) || selected)} /> : selected.area === "sources" ? <DemoSourceOperations key={selected.id} item={selected} onSave={update} onEdit={() => { setEditing(selected); setWizard("sources"); setSelected(null); }} close={() => setSelected(null)} /> : selected.area === "connections" ? <DemoConnection key={selected.id} item={selected} records={records} onChange={(items, item) => { setRecords(items); setSelected(item); setNotice("Conexión actualizada en la demo."); }} onOpen={setSelected} /> : <><DocumentFile key={selected.id} id={selected.id} /><ResourceAccess key={selected.id} item={selected} records={records} onSave={update} /></>}</>}</DialogContent></Dialog>
